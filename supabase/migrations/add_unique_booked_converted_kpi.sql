-- Unique booked leads who eventually spoke to LO (show ∪ claimed ∪ live_transfer).
-- Book-to-conversation rate = unique_booked_converted ÷ unique_booked (computed in app).
...
COMMENT ON FUNCTION public.dashboard_kpi_counts(uuid[], timestamptz, timestamptz) IS
  'Dashboard KPI numerators; includes unique_booked_converted (booked ∩ spoken) for book-to-conversation rate.';

-- Per-client returns TABLE — drop and recreate to add unique_booked_converted column.
DROP FUNCTION IF EXISTS public.dashboard_kpi_counts_by_client(uuid[], timestamptz, timestamptz);

CREATE FUNCTION public.dashboard_kpi_counts_by_client(
  p_client_ids uuid[] DEFAULT NULL,
  p_start timestamptz DEFAULT NULL,
  p_end timestamptz DEFAULT NULL
)
...
AS $$
  ...
  earliest_book AS (
    SELECT client_id, lead_key, MIN(occurred_at) AS first_book
    FROM scoped
    WHERE event_type = 'appointment_booked'
      AND lead_key IS NOT NULL
      AND client_id IS NOT NULL
    GROUP BY client_id, lead_key
  ),
  earliest_claim AS (
    SELECT client_id, lead_key, MIN(occurred_at) AS first_claim
    FROM scoped
    WHERE event_type = 'claimed'
      AND lead_key IS NOT NULL
      AND client_id IS NOT NULL
  ...
  booked_converted AS (
    ...
    FROM (
      SELECT DISTINCT client_id, lead_key
      FROM scoped
      WHERE event_type = 'appointment_booked'
        AND lead_key IS NOT NULL
        AND client_id IS NOT NULL
  ...
  SELECT
    ...
    COUNT(*) FILTER (WHERE s.event_type = 'appointment_rescheduled'),
...
COMMENT ON FUNCTION public.dashboard_kpi_counts_by_client(uuid[], timestamptz, timestamptz) IS
  'Per-client KPI numerators; includes unique_booked_converted (booked ∩ spoken).';
