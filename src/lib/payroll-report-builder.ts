...
export async function buildUnifiedPayrollReport(
  service: ServiceClient,
  startDate: string,
  endDate: string,
): Promise<{ report: UnifiedPayrollReport } | { error: string }> {
  const [
  ...
  if (leadIds.length > 0) {
    const { data: leads } = await service.from('acquisition_leads').select('id, lead_name').in('id', leadIds);
