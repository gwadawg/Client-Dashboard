...
export function validateSharedResponses(raw: Record<string, unknown>): {
  ...
  const done_for_day = asBool(raw.done_for_day);
  const done_for_day_note = String(raw.done_for_day_note ?? '').trim();
  if (!done_for_day && !done_for_day_note) {
    return { ok: false, error: 'If not done for the day, add a short note.' };
  }
  return {
    ok: true,
    shared: {
      ...
      productivity_rating,
      done_for_day,
      ...(done_for_day_note ? { done_for_day_note } : {}),
    },
  };
}
...
export function validateDepartmentCustom(
  ...
  const custom: CcmEodCustom = {
    ...
    setters_on_time: asBool(raw.setters_on_time),
    attendance_notes: String(raw.attendance_notes ?? '').trim(),
    dial_targets_vs_hit: String(raw.dial_targets_vs_hit ?? '').trim(),
  ...
  if (!custom.dial_targets_vs_hit) {
    return { ok: false, error: 'Note dial / booking targets vs hit.' };
  }
