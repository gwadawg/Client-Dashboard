...
import { hasPermission, canViewClientRevenue, canAccessAutomations, type AllowedPermissions } from './permissions';
...
/** Automations tab + /api/slack/* — explicit grant or any Admin sidebar tab. */
export function requireAutomationsAccess(ctx: AuthContext): NextResponse | null {
  if (canAccessAutomations({ isOwner: ctx.isOwner, allowedPermissions: ctx.allowedPermissions })) {
    return null;
  }
  return NextResponse.json({ error: 'Forbidden' }, { status: 403 });
}
