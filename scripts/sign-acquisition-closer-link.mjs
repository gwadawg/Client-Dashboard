#!/usr/bin/env node
/**
 * Print a signed Closer form magic link for ops / testing.
 *
 *   node scripts/sign-acquisition-closer-link.mjs CONTACT_ID [GHL_APPOINTMENT_ID]
 *
 * Omit GHL_APPOINTMENT_ID when the close happened without a calendar appointment.
 */
...
function buildUrl(baseUrl, contactId, appointmentId) {
  ...
  return `${baseUrl.replace(/\/$/, '')}/forms/acquisition/closer?${params.toString()}`;
}
...
if (!contactId) {
  console.error('Usage: node scripts/sign-acquisition-closer-link.mjs CONTACT_ID [GHL_APPOINTMENT_ID]');
  process.exit(1);
}
